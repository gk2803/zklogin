...
function App() {
  ...
  const [formAddress, setFormAddress] = useState("0xfa0f8542f256e669694624aa3ee7bfbde5af54641646a3a05924cf9e329a8a36");
              ...
              <Box sx={{ p: "2px", borderRadius: 2, mt: "10px", border: '3px solid', borderColor: 'divider', bgcolor: 'background.paper' }}>
                ...
                <form
                  onSubmit={(e) => {
                    ...
                    // validate address
                    if (validateAddress(address) === false) return;
                    ...
                    if (BigInt(addressBalance?.totalBalance) < toMist(amount)) {
                      enqueueSnackbar(`Insufficient SUI `, { variant: "error" });
                      return;
                    }

                    setPendingTx({ address, amount });
                    setConfirmOpen(true);
                  }}
                  style={{ display: 'flex', flexDirection: 'column', gap: '16px', width: '620px' }}
                >
                  ...
                  <Box>
                    ...
                    <div className='inputWithButton'>
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          if (validateAddress(formAddress) === false) return;
                          window.open(`https://explorer.polymedia.app/address/${formAddress}?network=devnet`, "_blank");
                        }}
                      >
                        <img src={search} style={{ width: '20px', height: '20px', marginTop: '8px', marginRight: '8px' }} >
                        </img>
                ...
                <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)}>
                  ...
                  <DialogContent>
                    <Typography>
                      Are you sure you want to send {pendingTx?.amount} SUI to {pendingTx?.address}?<br />
                  ...
                  <DialogActions>
                    ...
                    <Button onClick={async () => {
                      setConfirmOpen(false);
                      const { amount, address } = pendingTx!;
                      setPendingTx(null);
                      try {
                      if (
                        !ephemeralKeyPair ||
                      ...
                      const [coin] = txb.splitCoins(txb.gas, [toMist(amount)]);
